...
pub async fn get_teacher_details(
    ...
    get_teacher_details_db(&app_state.db, teacher_id)
        .await
        .map(|teacher| HttpResponse::Ok().json(teacher_id))
}
...
mod tests {
    ...
    #[actix_rt::test]
    async fn get_all_teachers_sucess_test() {
        dotenv().ok();
        let db_url = env::var("DATABASE_URL").expect("DATABASE is not setting in .env file");
    ...
    async fn post_teacher_success_test() {
        ...
        let new_teacher = CreateTeacher {
            name: "Third Teacehr".into(),
            picture_url: "test_url".into(),
