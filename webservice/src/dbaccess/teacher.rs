...
pub async fn update_teacher_details_db(
    ...
    let temp = Teacher {
        id: row.id,
        name: if let Some(name) = update_teacehr.name {
            name
        } else {
            row.name
        },
        picture_url: if let Some(pic) = update_teacehr.picture_url {
            pic
        } else {
            row.picture_url
        },
        profile: if let Some(profile) = update_teacehr.profile {
            profile
        } else {
            row.profile
        },
    ...
    let updated_row = sqlx::query!(
        ...
        temp.name, temp.picture_url, temp.profile, teacher_id
    )
        .fetch_one(pool)
        .await
        .map(|r| Teacher {
